...
@section('list', 'รายการข้อมูล Tag Delivery')
@section('content')
    <div class="container">
        <div class="form-group row">
            <div class="col">
                <button class="btn btn-success btn-ymt-create" onclick="location='{{ route('pages.delivery.create') }}'" id="create">เพิ่ม</button>
            </div>
        </div>
        <table class="tablesorter">
            ...
            <tbody>
                ...
                <tr>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $data->firstItem() + $key }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}"><img src="{{ $i->barcode_id }}"></a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->model_name }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->part_no }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->part_name }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->quantity }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->customer_name }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->created_at }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->updated_at }}</a></td>
                    <td><a href="{{ route('pages.delivery.edit',['id'=> $i->id]) }}">{{ $i->is_enable }}</a></td>
                    <td><a href="{{ route('delivery.pdf',['id'=> $i->id]) }}" target="_blank">pdf</a></td>
                </tr>
                @endforeach
            <tbody>
        </table>
