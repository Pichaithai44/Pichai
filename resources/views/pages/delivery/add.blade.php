...
@section('list', 'เพิ่มข้อมูล Tag Delivery')
@section('content')
    <div class="container form-control">
        <form action="{{ route('pages.delivery.create') }}" id="my_form" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            ...
            <div class="form-group row">
                ...
                <div class="col-8">
                    <input class="form-control text-left" type="text" name="quantity" value="{{ old('quantity') }}"/>
                </div>
            </div>
...
@section('script')
    <script>
        ...
        $(function()
        {
            ...
            source: "{{ route('pages.delivery.autocomplete') }}",
            minlenght:3,
            autoFocus:true,
