...
@section('list', 'เพิ่มข้อมูลค่าเริ่มต้น ฝ่ายงานย่อยในแผนก')
@section('content')
    <div class="container form-control">
        <form action="{{ route('pages.subdepartment.create') }}" id="my_form" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
        ...
        <div class="row">
            <div class="offset-4 col-8">
                ...
                <button class="btn btn-ymt" onclick="location='{{ route('pages.sub_department.index') }}'">กลับ</button>
            </div>
        </div>
    </div>
@endsection
