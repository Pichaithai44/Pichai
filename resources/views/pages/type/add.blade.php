@extends('layouts.app')
@section('title', 'Type')
@section('list', 'เพิ่มข้อมูลค่าเริ่มต้น Type')
@section('content')
    <div class="container form-control">
        <form action="{{ route('pages.type.create') }}" id="my_form" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
        ...
        <div class="row">
            <div class="offset-4 col-8">
                ...
                <button class="btn btn-ymt" onclick="location='{{ route('pages.type.index') }}'">กลับ</button>
            </div>
        </div>
    </div>
@endsection
